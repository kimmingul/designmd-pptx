...
- **More engine-solved patterns** (v2.0, #9) — `matrix_2x2` and `kpi_row` join the constraint engine (6 patterns total): the content grid is solved for adaptive card heights + text-fit while decorative chrome (title, axis labels) stays fixed. Each migration is verified against the geometry-contract harness **and** OfficeCLI's `view issues` render gate (which caught a real KPI-value overflow the harness missed) and an adversarial code review
