...
- **More engine-solved patterns** (v2.0, #9) — `matrix_2x2`, `kpi_row`, `pricing`, and `team` join the constraint engine (**8 patterns total**): the content grid is solved for adaptive card heights + text-fit while decorative chrome (title, axis labels) stays fixed, and non-flex elements (team's centered avatar disc) are overlaid post-solve from the solved card geometry. Each migration is verified against the geometry-contract harness **and** OfficeCLI's `view issues` render gate (which caught real value/name overflows the harness missed) **and** an adversarial code review + visual contact-sheet inspection
