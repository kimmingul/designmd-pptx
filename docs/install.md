...
Installs official `officecli@` **pin** from
`python/designmd_pptx/compatibility.json` via npm, and PyYAML via pip when
missing. The legacy shape-level binary is **manual** (release URL printed).
